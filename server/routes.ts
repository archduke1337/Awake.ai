...
// Create an API router where routes are defined without the `/api` prefix.
// The router can be mounted under `/api` by the full server, or by the
// Vercel function which will mount it at runtime.
...
export async function registerRoutes(app: Express): Promise<Server> {
  // Mount the API router under /api for the full server
